...
### `Задача от 2024-07-29`
- **Задача:** Спланировать и реализовать фичу ручного обновления прогресса студентов.
- **Статус:** 🟢 Выполнено
- **Подзадачи:**
    - [x] **Планирование:** Описать план реализации в `docs/APP_LOGIC_DOCS/Admin_Panel/add_progress_to_users_feature.md`.
    - [x] **Реализация:** Внедрить UI и логику в админ-панель.
        - [x] **Шаг 1: Расширение useStudentActions** - Добавить функции для управления прогрессом уроков
            - [x] Добавить `setLessonProgress(studentId, lessonId, isCompleted)` 
            - [x] Добавить `markLessonAsCompleted(studentId, lessonId)`
            - [x] Добавить `markLessonAsIncomplete(studentId, lessonId)`
            - [x] Обновить интерфейс `StudentActionsResult`
        - [x] **Шаг 2: Исправление useStudentDetails** - Загрузка всех уроков курса  
            - [x] Изменить логику загрузки: вместо только уроков с прогрессом - все уроки курса
            - [x] Мержить существующий прогресс со всеми уроками
            - [x] Убедиться что неначатые уроки показываются с is_completed: false
        - [x] **Шаг 3: Обновление UI в StudentCard** - Элементы управления прогрессом
            - [x] Добавить массовые операции на трёх уровнях (глобально, по ступеням, индивидуально)
            - [x] Реализовать условную логику кнопок: "Завершить" для незавершённых, "Сбросить" для завершённых
            - [x] Добавить кнопки "Отметить все как пройденные" и "Снять все отметки" глобально
            - [x] Добавить кнопки "Завершить все" и "Сбросить все" для каждой ступени
            - [x] Интегрировать вызовы новых функций из useStudentActions
            - [x] Добавить состояния загрузки и блокировку UI во время операций
...
### ✨ **[UX]** Внедрение Ripple-эффекта
- 🟢 **Этап 1: Создание централизованного компонента `<Ripple />`**
  - 🟢 Создать директорию `src/components/ui/Ripple/`.
  - 🟢 Внутри создать `Ripple.tsx` (логика компонента-обертки) и `Ripple.css` (стили для анимации с использованием Tailwind `@apply`).
  - 🟢 Реализовать компонент согласно плану в `docs/APP_LOGIC_DOCS/UI_UX/UI_UX_imporovments_feature.md`.
- 🟢 **Этап 2: Интеграция в `MainPage.tsx`**
  - 🟢 Импортировать компонент `Ripple` в `src/pages/MainPage/MainPage.tsx`.
  - 🟢 **Детализация:** На странице есть два типа кликабельных элементов, которые получат эффект.
  - 🟢 **1. Ссылка на баллы:** Обернуть компонент `Link`, ведущий на страницу `/points`.
    ```jsx
    // Было:
    <Link to={'/points'} className={'... rounded-full'}>
      ...
    </Link>
    ...
    ```
  - 🟢 **2. Карточки ступеней:** Внутри цикла `.map(stage => ...)` обернуть компонент `Link`, который представляет собой карточку ступени.
    ```jsx
    // Было:
    <Link key={stage.stage_id} to={`/library/stage/${stage.stage_id}`} className={...}>
        ...
    </Link>
    ...
    ```
  - 🟢 Убедиться, что для заблокированных карточек (`stage.is_unlocked: false`) эффект не срабатывает, так как на них стоит `pointer-events-none`.

### 🟢 **[Фикс]** Исправление кнопки "Назад" в Telegram
- 🟢 **Проблема:** При переходах по TabBar (профиль, библиотека, главная) в Telegram появлялась нежелательная кнопка "Назад".
- 🟢 **Решение:** Добавлен `back={false}` для основных страниц навигации (`MainPage`, `CommonPage`, `UserPage`) - эти страницы являются корневыми разделами и не должны показывать кнопку возврата.
