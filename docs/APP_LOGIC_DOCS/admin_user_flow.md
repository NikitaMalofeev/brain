...
- **Правая панель - Форма проверки:**
  ...
  - **Для уже проверенных сабмитов (approved/rejected):**
    ...
    - Кнопка "Изменить решение" (для админов и кураторов)
    - При нажатии открывается форма редактирования:
      - Новый статус (approved ↔ rejected)
      - Корректировка баллов (с автоматическим пересчетом у пользователя)
      - Причина изменения решения (обязательное поле)
      - Предупреждение: "Баллы пользователя будут пересчитаны"
...
- **Дедлайн сдачи задания (`deadline_at`):**
  ...
  - Используется для определения просроченных сдач (статус 'late')
- Кнопки: Сохранить, Отмена
