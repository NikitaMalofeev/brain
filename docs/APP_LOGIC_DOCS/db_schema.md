...
*Обновлено: 30.06.2025 - Исправлена архитектура привязки чатов к курсам (один-к-одному)*

## Обзор

База данных приложения "Brain Programming" построена на PostgreSQL через Supabase и содержит 23 основные таблицы и 1 бакет в Storage для управления пользователями, курсами, медиа-контентом, прогрессом, тарифами и доступом.
...
### 14. `chats` - Telegram-чаты ✅ ОБНОВЛЕНО 30.06.2025
...
- `order_num` (int4, default: 0, NOT NULL) - Порядковый номер для сортировки
- `created_at` (timestamptz, default: now(), NOT NULL) - Время создания
- `course_id` (uuid, FK → courses.id, nullable) - Привязка чата к конкретному курсу (один-к-одному)
...
### `can_user_access_stage(p_user_id UUID, p_stage_id BIGINT)` ✅ ОБНОВЛЕНО
**Назначение:** Комплексная функция для проверки доступа пользователя к этапу.
**Логика:**
1.  **Проверка роли:** Если роль пользователя 'admin' или 'curator', немедленно возвращает `true`, предоставляя полный доступ.
2.  **Проверка общего доступа:** Проверяет, не истек ли `access_till` у пользователя.
3.  **Проверка лимитов тарифа:** Анализирует `tariff_limits` для тарифа пользователя и данного этапа.
4.  **Проверка предварительных условий:** Если для тарифа установлено `requires_full_prereq`, вызывает функцию `check_all_previous_lessons_completed` для проверки завершения всех предыдущих уроков.
**Возвращает:** `BOOLEAN`
...
### `promote_user_to_curator(p_user_id UUID, p_web_login TEXT, p_web_password TEXT, p_admin_id UUID)` ✅ НОВАЯ ФУНКЦИЯ
**Назначение:** Повышает роль существующего пользователя до 'curator'.
**Логика:**
1.  **Проверка прав:** Убеждается, что пользователь с `p_admin_id` имеет роль 'admin'.
2.  **Проверка цели:** Проверяет, что целевой пользователь существует и не является уже куратором или админом.
3.  **Обновление роли и учетных данных:** Устанавливает `role = 'curator'`, `web_login` и хешированный `web_password_hash`.
4.  **Очистка связей:** Удаляет запись из `user_curator`, если повышаемый пользователь был чьим-то учеником.
**Возвращает:** `void`

### `get_available_chats_for_user(p_user_id UUID)` ✅ НОВАЯ ФУНКЦИЯ
**Назначение:** Получает список чатов, доступных конкретному пользователю на основе его активного тарифа И зачисления на курсы.
**Логика:**
1. **Проверка тарифа:** Находит активный тариф пользователя через `user_tariffs`.
2. **Проверка курсов:** Находит активные зачисления пользователя через `user_course_enrollments`.
3. **Фильтрация чатов:** Возвращает только те чаты, которые привязаны И к тарифу пользователя (через `tariff_chat_access`), И к курсу, на который он зачислен (через `chats.course_id`).
4. **Оптимизация:** Использует `INNER JOIN`-ы для производительности, `DISTINCT` для исключения дубликатов.
...
- `link` (text) - Ссылка на чат
- `order_num` (integer) - Порядковый номер для сортировки
- `course_name` (text) - Название курса (из JOIN с таблицей courses)
...
courses → chats (через course_id, один-к-одному)
