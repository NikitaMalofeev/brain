Ниже дам полную схему управления пользователями в админке с учётом наличия отдельных таблиц материалов и их просмотров. Делал упор на два таба («Ученики» и «Кураторы») и карточки, при этом добавлю раздел по библиотечным материалам, учитывая таблицы `materials` и `user_material_views`.
...
* **Бэкенд-логика** — будет реализована через React Hooks и SQL функции в Supabase.
* **Архитектура**: Используется тот же паттерн, что и в существующих хуках проекта (`useCoursesAdmin`, `useStagesAdmin`, etc.).
* **Поведение**:
  ...
  3. Вкладка «Кураторы» и карточка «Куратор» с простым списком "мои ученики".
