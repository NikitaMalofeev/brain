...
*Обновлено: 30.06.2025 - Добавлен флоу повышения до куратора*
---

## 🔒 Права доступа в админке

В Brain Programming существует два типа административных ролей с различными уровнями доступа к **управлению** контентом.

| Действие (Управление)        | Admin | Curator |
| ---------------------------- | ----- | ------- |
| Проверка ДЗ                  | ✅     | ✅       |
...
**Важное примечание по доступу к просмотру:**
- **Администраторы и кураторы** имеют **полный доступ к просмотру** всех учебных материалов (ступеней и уроков) без ограничений по тарифам. Это позволяет им свободно перемещаться по контенту для поддержки учеников.
...
1.  **Основная информация:**
    ...
    *   **✅ Роль пользователя:** Отображение текущей роли и кнопка для повышения до куратора (только для админов).
...
### 2.A.0 Создание нового куратора ✅ РЕАЛИЗОВАНО
...
### 2.A.1 Повышение пользователя до куратора ✅ РЕАЛИЗОВАНО

В дополнение к созданию кураторов с нуля, администраторы могут повысить любого существующего пользователя до роли куратора.

**Где находится:**
- На детальной карточке ученика (`/admin/students/{studentId}`).
- В блоке "Основная информация" есть раздел "Роль пользователя", где отображается текущая роль и кнопка **"Сделать куратором"**.

**User Flow:**
1.  **Нажатие на кнопку:** Администратор нажимает "Сделать куратором".
2.  **Модальное окно:** Появляется модальное окно, аналогичное окну создания куратора. Поля "Имя", "Фамилия" и "Telegram Username" предзаполняются из данных ученика.
3.  **Ввод учетных данных:** Администратор должен ввести **уникальный логин** и **пароль** для веб-доступа нового куратора.
4.  **Подтверждение:** После нажатия "Сохранить" система:
    - Вызывает RPC-функцию `promote_user_to_curator`.
    - Обновляет роль пользователя на 'curator'.
    - Устанавливает `web_login` и хешированный `web_password_hash`.
    - Удаляет пользователя из списков учеников, закрепленных за другими кураторами (если он был там).
5.  **Отображение учетных данных:** После успешного повышения появляется модальное окно с логином и паролем нового куратора и кнопками для их копирования.

**Техническая реализация:**
- Компонент: `StudentCard.tsx`.
- Хук: `useCuratorsAdmin.promoteToCurator()`.
- SQL функция: `public.promote_user_to_curator(p_user_id, p_web_login, p_web_password, p_admin_id)`.

### 2.A.2 Список кураторов
