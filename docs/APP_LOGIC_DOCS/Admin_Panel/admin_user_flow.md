...
graph TD;
    ...
    D --> D1["<b>Основная инфо:</b><br/>- ФИО, Контакты<br/>- Регистрация, Посл. вход<br/>- Баллы"];
...
## 2.A. Кураторы (/admin/curators) 🟡 В РАЗРАБОТКЕ (Фронтенд)
...
3.  **Действия на карточке куратора:**
    ...
    *   **Кнопка «Назначить ученика»:**
        ...
        *   В модальном окне отображается список всех пользователей с ролью 'user', которые еще не закреплены за данным куратором (или вообще не имеют куратора).

        *   Возможность поиска/фильтрации учеников в модальном окне по ФИО или Telegram ID.
            - **Техническая реализация:** Поиск на клиенте с использованием `Fuse.js`
...
graph TD;
  A["Вкладка 'Кураторы'"] --> B["Список кураторов<br/>- ФИО<br/>- Telegram-ID / Web-login<br/>- Кол-во учеников<br/>- Кнопка 'Открыть'"];
