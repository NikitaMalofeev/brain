...
-   **Добавление/удаление баллов:** Форма для ввода величины корректировки и кнопки «Добавить баллы» и «Удалить баллы» для изменения текущего баланса студента.
...
2. **`src/pages/AdminPage/components/StudentsManager/StudentCard.tsx`** - ✅ **Выполнено** - Модификация UI для добавления элементов управления прогрессом и проектирование формы добавления/удаления баллов
...
### ✅ Шаг 4: UI для добавления/удаления баллов

**Файл:** `src/pages/AdminPage/components/StudentsManager/StudentCard.tsx`

1. ✅ **Добавлена форма рядом с информацией о баллах:**
   - Поле ввода `deltaPoints` (тип: number) для указания величины корректировки баллов.
   - Кнопка «Добавить баллы» (`action-btn edit-btn`) для увеличения `basicInfo.total_points` на `deltaPoints`.
   - Кнопка «Удалить баллы» (`action-btn delete-btn`) для уменьшения `basicInfo.total_points` на `deltaPoints` (не ниже 0).
2. ✅ **Реализована валидация ввода:** проверка `deltaPoints > 0` и показ сообщения об ошибке при некорректном значении.
3. ✅ **Добавлены уведомления:** после успешной операции вызывается `loadStudentDetails(studentId)` для обновления UI и показывается уведомление об успехе с новым балансом.
4. ✅ **Добавлено состояние загрузки:** `pointsLoading` блокирует поле ввода и кнопки во время выполнения операции.
5. ✅ **Использована существующая функция:** `updateStudentPoints` из `useStudentActions` вместо создания новых функций в хуке.
...
4. **✅ Гибкое управление баллами:**
   - Абсолютное изменение баллов (кнопка "Изменить")
   - Относительное добавление/удаление баллов через форму
   - Валидация ввода и защита от отрицательных значений
5. **✅ Интуитивный UX** - кнопки меняются в зависимости от статуса урока
6. **✅ Надёжная обработка ошибок** - с подтверждениями и информативными сообщениями
7. **✅ Блокировка UI** - во время операций интерфейс корректно блокируется
...
- **Гибко управлять баллами студентов:**
  - Устанавливать абсолютное значение баллов (кнопка "Изменить")
  - Добавлять баллы за выполненные задания или активность
  - Удалять баллы при необходимости (например, за нарушения)
  - Безопасно корректировать баланс с защитой от отрицательных значений