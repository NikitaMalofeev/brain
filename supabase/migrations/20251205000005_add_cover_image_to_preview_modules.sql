-- Migration: Add cover_image to preview modules function
-- Date: 2025-12-05
-- Description: Добавляем module_cover_image в функцию get_preview_stream_modules

-- Сначала удаляем старую функцию (изменилась структура возвращаемых данных)
DROP FUNCTION IF EXISTS public.get_preview_stream_modules();
...
BEGIN
    ...
    SELECT
        ...
        sm.cover_image AS module_cover_image,
        ...
        COALESCE(mud.unlock_offset_days, (sm.order_num - 1) * 7) AS unlock_day,
...
'Получить модули для preview режима (гости). Возвращает модули первого активного потока с обложками и unlock_offset_days.';
