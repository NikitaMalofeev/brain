-- Migration: Fix copy_stream_with_config to copy new fields
-- Date: 2025-12-07
-- Description: Добавление копирования grid_days и technique_ids
...
BEGIN
  ...
  -- 5. Копируем блоки уроков (lesson_blocks) с technique_ids
  ...
  LOOP
    ...
    IF v_new_lesson_id IS NOT NULL THEN
      ...
      VALUES (
        ...
        COALESCE(v_block_record.technique_ids, '{}'),
        v_block_record.meta_json
      );
    END IF;
  END LOOP;

  RAISE NOTICE 'Copied lesson_blocks with technique_ids';
  ...
  -- 9. Копируем модули тарифов (tariff_stream_modules) с grid_days
  FOR v_tsm_record IN
    SELECT tsm.*
  ...
  LOOP
    ...
    IF v_new_stream_tariff_id IS NOT NULL AND v_new_module_id IS NOT NULL THEN
      ...
      RAISE NOTICE 'Copied tariff_stream_module with grid_days=%, unlock_offset_days=%',
        v_tsm_record.grid_days, COALESCE(v_tsm_record.unlock_offset_days, 0);
    END IF;
  END LOOP;
...
- Блоки уроков (lesson_blocks) с technique_id И technique_ids (массив)
- Задания (assignments)
- События календаря
- Тарифы и их настройки (tariff_stream_modules) с grid_days
...
BEGIN
  ...
  RAISE NOTICE '  - Добавлено копирование grid_days в tariff_stream_modules';
  RAISE NOTICE '  - Добавлено копирование technique_ids в lesson_blocks';
END $$;
