-- Migration: Add overdue_assignments to get_user_stream_modules
-- Date: 2025-11-27
-- Description: Добавить подсчёт просроченных заданий
...
RETURNS TABLE (
    ...
    overdue_assignments BIGINT  -- НОВОЕ: просроченные задания
) AS $$
DECLARE
    v_stream_id UUID;
    v_tariff_id UUID;
...
BEGIN
    ...
    -- Уроки потока пользователя с offset'ами
    stream_lessons AS (
        SELECT
...
Включает подсчёт заданий: total_assignments, completed_assignments, overdue_assignments';
