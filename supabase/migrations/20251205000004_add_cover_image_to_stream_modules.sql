-- Migration: Add cover_image to stream_modules
-- Date: 2025-12-05
-- Description: Добавляем возможность загружать обложки для модулей

-- Добавляем колонку cover_image в таблицу stream_modules
ALTER TABLE public.stream_modules
ADD COLUMN IF NOT EXISTS cover_image TEXT;

COMMENT ON COLUMN public.stream_modules.cover_image IS 'URL обложки модуля (загружается в storage)';

-- Обновляем RPC функцию get_user_stream_modules чтобы возвращала cover_image
...
BEGIN
  ...
  assignment_stats AS (
    SELECT
      l.stream_module_id,
      COUNT(CASE WHEN l.has_assignment = true THEN 1 END)::INT as total_assignments_count,
      COUNT(DISTINCT CASE
        WHEN l.has_assignment = true
          AND EXISTS (
            SELECT 1 FROM submissions s
...
'Возвращает модули потока пользователя с учётом его тарифа и обложками модулей.';
