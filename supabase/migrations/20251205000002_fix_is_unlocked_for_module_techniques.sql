-- Migration: Fix is_unlocked for module techniques
-- Date: 2025-12-05
-- Description: Исправить логику is_unlocked для техник из модулей
--   Если техника из модуля и модуль ещё не открылся - is_unlocked должен быть false
...
BEGIN
  ...
  SELECT
    ...
    (m.status IN ('purchasable', 'free') AND NOT COALESCE(ua.has_acc, false)
     AND m.unlock_condition_type IS NULL
     AND NOT COALESCE(ua.has_future_acc, false)) as can_purchase,
    -- ИСПРАВЛЕНО: is_unlocked
    CASE
      -- Техника из модуля: разблокирована только если has_acc = true
      WHEN ua.src = 'module' THEN COALESCE(ua.has_acc, false)
      -- Техника с будущим доступом из модуля (ещё не открылась)
      WHEN COALESCE(ua.has_future_acc, false) AND NOT COALESCE(ua.has_acc, false) THEN false
      -- Уже есть доступ - разблокирована
      WHEN COALESCE(ua.has_acc, false) THEN true
      -- Есть условие разблокировки - заблокирована
      WHEN m.unlock_condition_type IS NOT NULL THEN false
      -- Самостоятельная техника без привязки к модулю - разблокирована
      WHEN m.is_standalone = true AND ua.material_id IS NULL THEN true
      -- Техника привязана к модулю но нет записи доступа - заблокирована
      WHEN ua.material_id IS NULL AND m.is_standalone = false THEN false
      -- По умолчанию - разблокирована (для standalone без привязок)
      ELSE true
    END as is_unlocked,
    ...
    -- ИСПРАВЛЕНО: user_access_source - показываем 'module' даже если ещё нет доступа
    CASE
      WHEN ua.src IS NOT NULL THEN ua.src
      WHEN ua.has_future_acc THEN 'module'
      ELSE NULL
    END as user_access_source,
...
ИСПРАВЛЕНО: is_unlocked корректно показывает false для техник из модулей, которые ещё не открылись.';
