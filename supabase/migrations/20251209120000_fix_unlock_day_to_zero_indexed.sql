-- Migration: Fix unlock_day to return 0-indexed offset
-- Date: 2025-12-09
-- Description: Исправляем get_user_stream_modules чтобы возвращать unlock_offset_days напрямую (0-indexed)
--   Это унифицирует подход с get_user_techniques_with_schedule и убирает путаницу
--   0 = сразу доступен (в день старта потока)
--   7 = через 7 дней после старта
...
BEGIN
  ...
  WITH module_data AS (
    SELECT
      ...
      tsm.access_duration_days as duration_days,
  ...
  SELECT
    ...
    -- ИСПРАВЛЕНО: возвращаем unlock_offset напрямую (0-indexed)
    -- Было: md.unlock_offset + 1
    -- Стало: md.unlock_offset
    -- 0 = сразу доступен, 7 = через 7 дней
    md.unlock_offset,
    md.first_stage,
...
'Возвращает модули потока пользователя. unlock_day теперь 0-indexed (0 = сразу доступен).';
