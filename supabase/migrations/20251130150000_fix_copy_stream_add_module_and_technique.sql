-- Migration: Fix copy_stream_with_config - add stream_module_id for lessons and technique_id for blocks
-- Date: 2025-11-30
-- Description: Обновление функции копирования потока для поддержки:
--   1. stream_module_id в уроках (привязка урока к модулю)
--   2. technique_id в блоках (привязка техники к блоку)
...
BEGIN
  ...
  -- 4. Копируем уроки (lessons) - ДОБАВЛЕНО: stream_module_id для привязки к новому модулю
  ...
  LOOP
    ...
    IF v_new_stage_id IS NOT NULL THEN
      INSERT INTO public.lessons (
        stage_id,
        stream_module_id,  -- ДОБАВЛЕНО: привязка урока к модулю
        stream_id,
        name,
        description,
        order_num,
      ...
      VALUES (
        v_new_stage_id,
        v_new_module_id,  -- ДОБАВЛЕНО: новый module_id
        v_new_stream_id,
        ...
        -- ТОЧНОЕ копирование open_day_offset (без автоматических fallback)
        v_lesson_record.open_day_offset,
        -- ТОЧНОЕ копирование deadline_day_offset (без автоматических fallback)
        v_lesson_record.deadline_day_offset,
        -- Даты обнуляем, так как у нового потока другая start_date
        NULL,
        NULL
      )
  ...
  -- 5. Копируем блоки уроков (lesson_blocks) - ДОБАВЛЕНО: technique_id
  ...
  LOOP
    ...
    IF v_new_lesson_id IS NOT NULL THEN
      INSERT INTO public.lesson_blocks (
        ...
        order_num,
        material_id,
        technique_id,  -- ДОБАВЛЕНО: привязка техники к блоку
        meta_json
      )
      VALUES (
        v_new_lesson_id,
        ...
        v_block_record.technique_id,  -- ДОБАВЛЕНО: копируем technique_id
  ...
  RAISE NOTICE 'Stream copy completed: % -> % (with % assignments)', p_source_stream_id, v_new_stream_id, v_assignments_copied;
...
BEGIN
  ...
  RAISE NOTICE '  - stream_module_id копируется для уроков';
  RAISE NOTICE '  - technique_id копируется для блоков';
END $$;
