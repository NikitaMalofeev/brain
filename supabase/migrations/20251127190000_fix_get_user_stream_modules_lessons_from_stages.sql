-- Migration: Fix get_user_stream_modules - get lessons from course_stages
-- Date: 2025-11-27
-- Description: Уроки берутся через lessons -> course_stages -> stream_modules, а не через calendar_events
...
BEGIN
    ...
    -- Уроки для каждого модуля через course_stages
    module_lessons AS (
        SELECT
    ...
    module_stats AS (
        SELECT
            ...
            COUNT(DISTINCT CASE
                WHEN ml.open_at IS NOT NULL AND ml.open_at <= now() THEN ml.lesson_id
                WHEN ml.open_at IS NULL AND (ml.open_day_offset IS NULL OR ml.open_day_offset <= v_current_day) THEN ml.lesson_id
            END) AS unlocked_lessons,
            -- Просроченные: открыт, есть дедлайн, дедлайн прошёл, не выполнен
            COUNT(DISTINCT CASE
                WHEN (
                    (ml.open_at IS NOT NULL AND ml.open_at <= now()) OR
                    (ml.open_at IS NULL AND (ml.open_day_offset IS NULL OR ml.open_day_offset <= v_current_day))
                )
                AND ml.deadline_at IS NOT NULL
                AND ml.deadline_at < now()
...
Уроки берутся через lessons -> course_stages -> stream_modules.
...
BEGIN
    ...
    RAISE NOTICE '  - Уроки теперь берутся через course_stages, а не calendar_events';
