-- Migration: Add bundle_name to get_user_techniques_with_schedule
-- Date: 2025-11-27
-- Description: Добавляем название пакета (bundle) для группировки в библиотеке
...
BEGIN
  ...
  -- 2. Доступ через пакеты (bundles) - теперь с названием пакета
  bundle_access AS (
    SELECT DISTINCT
...
Добавлены поля bundle_id и bundle_name для группировки техник по пакетам.';
...
BEGIN
  ...
  RAISE NOTICE '  - Добавлены поля bundle_id и bundle_name';
END $$;
