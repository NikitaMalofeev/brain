-- Migration: Fix get_user_techniques_with_schedule function
-- Date: 2025-11-26
-- Description: Исправляем функцию для правильной работы библиотеки техник
-- Учитываем все источники доступа: прямой, пакеты, модули тарифов
-- Фильтруем техники модулей по активному тарифу пользователя (user_tariffs)

-- Удаляем старую версию функции
DROP FUNCTION IF EXISTS public.get_user_techniques_with_schedule(uuid);

-- Создаём исправленную функцию
...
'Возвращает техники для библиотеки с учётом всех источников доступа: прямой доступ (user_material_access), пакеты (user_bundles), модули тарифов (tariff_module_materials). Фильтрует по активному тарифу пользователя.';
...
BEGIN
  ...
  RAISE NOTICE '  - Прямой доступ через user_material_access';
  RAISE NOTICE '  - Доступ через пакеты (user_bundles -> bundle_techniques)';
  RAISE NOTICE '  - Доступ через модули тарифов (user_module_access -> tariff_module_materials)';
  RAISE NOTICE '  - Фильтрация по активному тарифу (user_tariffs.is_active = true)';
  RAISE NOTICE '  - Учёт unlock_offset_days и active_days для модульных техник';
END $$;
