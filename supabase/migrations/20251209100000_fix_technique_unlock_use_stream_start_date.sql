-- Migration: Fix technique unlock to use stream start_date instead of granted_at
-- Date: 2025-12-09
-- Description: Исправляем расчёт даты разблокировки техник:
--   Теперь используем streams.start_date вместо user_module_access.granted_at
--   Это позволяет корректно пересчитывать даты при изменении даты старта потока
...
BEGIN
  ...
  -- unlock_offset_days - это СМЕЩЕНИЕ в днях (0 = сразу доступен, 1 = через 1 день)
  -- Используем streams.start_date вместо granted_at для расчёта даты разблокировки
  module_access AS (
    SELECT
      tmm.material_id,
      'module'::text as src,
      CASE
        ...
        THEN uss.start_date + (COALESCE(tmm.unlock_offset_days, 0) || ' days')::interval + (tmm.active_days || ' days')::interval
        ELSE NULL
      END as expires_at,
      -- Дата разблокировки = start_date потока + unlock_offset_days
      uss.start_date + (COALESCE(tmm.unlock_offset_days, 0) || ' days')::interval as unlocks_at,
      uma.granted_at,
      sm.id as mod_id,
      sm.name as mod_name,
...
'Возвращает техники для библиотеки. Использует streams.start_date для расчёта дат разблокировки.';
