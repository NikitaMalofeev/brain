-- Migration: Fix copy_stream_with_config to include unlock_offset_days
-- Date: 2025-11-27
-- Description: Добавить копирование unlock_offset_days для модулей при копировании потока

-- Удаляем ВСЕ версии функции (с разными сигнатурами)
...
BEGIN
  ...
  -- Смещение для новых ступеней = максимальный order_num + 1000 (чтобы был запас)
  ...
  LOOP
    ...
    IF v_new_stream_tariff_id IS NOT NULL AND v_new_module_id IS NOT NULL THEN
      ...
      VALUES (
        ...
        COALESCE(v_tsm_record.unlock_offset_days, 0),  -- ДОБАВЛЕНО!
  ...
  RAISE NOTICE 'Copied tariff_stream_modules with unlock_offset_days';
...
'Полное копирование потока с конфигурацией. Копирует модули, ступени, уроки, блоки, события, тарифы, материалы и unlock_offset_days.';
...
BEGIN
  ...
  RAISE NOTICE '  - ДОБАВЛЕНО копирование unlock_offset_days для модулей тарифов';
  RAISE NOTICE '  - Теперь настройки "Открыть с дня потока" сохраняются при копировании';
END $$;
