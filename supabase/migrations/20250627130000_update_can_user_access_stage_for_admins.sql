...
BEGIN
    ...
    -- Шаг 6: Получаем лимиты для этого тарифа и этапа (ВОССТАНОВЛЕНО - оригинальная логика)
    SELECT * INTO v_limit
    FROM public.tariff_limits
    WHERE tariff_id = user_tariff_id AND stage_id = p_stage_id;
    
    -- ВАЖНО: Если для тарифа нет никаких правил по этому этапу, доступа НЕТ (как в оригинале)
