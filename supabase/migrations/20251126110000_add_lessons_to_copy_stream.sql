-- Migration: Add lessons/stages copying to copy_stream function
-- Date: 2025-11-26
-- Description: Добавляем копирование ступеней (course_stages) и уроков (lessons) при копировании потока

-- Удаляем старую версию функции
DROP FUNCTION IF EXISTS copy_stream(UUID, TEXT, DATE);

-- Создаем улучшенную функцию с копированием уроков
...
BEGIN
  ...
  LOOP
    ...
    IF v_new_stage_id IS NOT NULL THEN
      ...
      RAISE NOTICE 'Copied lesson: % (stage % -> %)', v_lesson_record.name, v_lesson_record.stage_id, v_new_stage_id;
    END IF;
  END LOOP;

  RAISE NOTICE 'Copied lessons';

  -- 5. Копируем события календаря (с правильной структурой полей)
  ...
  -- 6. Копируем связи с тарифами (stream_tariffs)
  ...
  -- 7. Копируем модули тарифов (tariff_stream_modules)
  FOR v_tsm_record IN
    SELECT tsm.*
  ...
  -- 8. Копируем техники модулей тарифов (tariff_module_techniques)
  FOR v_tmt_record IN
    SELECT tmt.*
  ...
  -- 9. Копируем stream_module_techniques (если есть)
  ...
  RAISE NOTICE 'Copied stages: %, Lessons will be copied accordingly', jsonb_object_keys(v_old_to_new_stage_id);
...
'Полное копирование потока со всеми модулями, ступенями, уроками, событиями, тарифами и техниками. Даты сдвигаются на новую дату начала.';
...
DO $$
BEGIN
  RAISE NOTICE '✅ Функция copy_stream() обновлена с поддержкой уроков:';
