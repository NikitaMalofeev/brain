-- Migration: Fix copy_stream_with_config - preserve exact lesson offsets
-- Date: 2025-11-27
-- Description: Копировать точные значения open_day_offset и deadline_day_offset без автоматических fallback
...
BEGIN
  ...
  -- 4. Копируем уроки (lessons) - СОХРАНЯЕМ ТОЧНЫЕ ЗНАЧЕНИЯ open_day_offset и deadline_day_offset
  ...
  LOOP
    ...
    IF v_new_stage_id IS NOT NULL THEN
      ...
      RAISE NOTICE 'Copied lesson: % (open_day_offset=%, deadline_day_offset=%)',
        v_lesson_record.name,
        v_lesson_record.open_day_offset,
        v_lesson_record.deadline_day_offset;
    END IF;
...
- Уроки (lessons) с точными значениями open_day_offset и deadline_day_offset
- Блоки уроков (lesson_blocks)
- Задания (assignments)
- События календаря
...
BEGIN
  ...
  RAISE NOTICE '  - open_day_offset и deadline_day_offset копируются точно без fallback';
END $$;
