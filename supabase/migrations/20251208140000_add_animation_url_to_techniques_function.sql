-- Migration: Add animation_url to get_user_techniques_with_schedule
-- Date: 2025-12-08
-- Description: Добавляем поле animation_url в функцию для отображения анимации в плеере

-- Удаляем старую версию функции
DROP FUNCTION IF EXISTS public.get_user_techniques_with_schedule(uuid);

-- Создаём исправленную функцию с animation_url
...
RETURNS TABLE(
  ...
  cover_image text,
  audio_url text,
  animation_url text,  -- ДОБАВЛЕНО: URL mp4 анимации для плеера
...
'Возвращает техники для библиотеки с учётом всех источников доступа. Включает animation_url для плеера.';
...
BEGIN
  ...
  RAISE NOTICE '  - Добавлено поле animation_url для mp4 анимации в плеере';
END $$;
