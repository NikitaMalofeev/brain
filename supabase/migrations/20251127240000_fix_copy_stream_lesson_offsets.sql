-- Migration: Fix copy_stream to set correct lesson offsets
-- Date: 2025-11-27
-- Description: При копировании потока устанавливать open_day_offset = order_num - 1
...
BEGIN
  ...
  -- 4. Копируем уроки (lessons) - ИСПРАВЛЕНО: устанавливаем open_day_offset = order_num - 1
  ...
  LOOP
    ...
    IF v_new_stage_id IS NOT NULL THEN
      INSERT INTO public.lessons (
        ...
        cover_image_path,
        has_assignment,
        -- ИСПРАВЛЕНО: устанавливаем относительные offset'ы вместо абсолютных дат
        open_day_offset,
        deadline_day_offset,
        -- Абсолютные даты обнуляем - они будут рассчитываться по offset'ам
        open_at,
        deadline_at
      )
      VALUES (
        ...
        -- open_day_offset = order_num - 1 (День 1 = offset 0, День 2 = offset 1...)
        COALESCE(NULLIF(v_lesson_record.open_day_offset, 0), v_lesson_record.order_num - 1),
        -- deadline_day_offset = open_day_offset + 2 (всегда пересчитываем)
        COALESCE(NULLIF(v_lesson_record.open_day_offset, 0), v_lesson_record.order_num - 1) + 2,
        -- Абсолютные даты = NULL (рассчитываются по offset'ам)
        NULL,
        NULL
      )
      ...
      RAISE NOTICE 'Copied lesson: % (% -> %) with open_day_offset=%',
        v_lesson_record.name, v_lesson_record.id, v_new_lesson_id,
        COALESCE(v_lesson_record.open_day_offset, v_lesson_record.order_num - 1);
    END IF;
  END LOOP;

  RAISE NOTICE 'Copied lessons with correct offsets';
  ...
  -- 8. Копируем модули тарифов (tariff_stream_modules) с unlock_offset_days
  FOR v_tsm_record IN
    SELECT tsm.*
  ...
  RAISE NOTICE 'Stream copy completed: % -> %', p_source_stream_id, v_new_stream_id;
...
'Копирование потока с правильными offset''ами уроков.
Уроки получают open_day_offset = order_num - 1, deadline_day_offset = order_num + 1.
Абсолютные даты (open_at, deadline_at) обнуляются.';
...
BEGIN
  ...
  RAISE NOTICE '  - Уроки получают open_day_offset = order_num - 1';
  RAISE NOTICE '  - deadline_day_offset = order_num + 1 (открытие + 2 дня)';
  RAISE NOTICE '  - Абсолютные даты (open_at, deadline_at) обнуляются';
END $$;
