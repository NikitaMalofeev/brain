-- Исправляем конфликт имен столбцов в функции get_library_stages_optimized
...
BEGIN
  ...
  SELECT
    ...
    CASE 
      ...
      WHEN NOT EXISTS (
        SELECT 1 FROM public.tariff_limits tl
...
COMMENT ON FUNCTION public.get_library_stages_optimized IS 'Оптимизированная функция для получения этапов курса с информацией о доступности за один запрос. Исправлен конфликт имен столбцов.'; 